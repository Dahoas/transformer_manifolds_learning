...
save_model = False # if True, save model after each eval
...
# data
dataset = 'openwebtext_1_100'
gradient_accumulation_steps = 6 * 5 * 8 # used to simulate larger batch sizes
batch_size = 2 # if gradient_accumulation_steps > 1, this is the micro-batch size
batch_mode = 'random' # linear
block_size = 1024
# model
n_layer = 12
n_head = 12
...
forward_step = 0
def get_batch(split, batch_mode='random'):
    global forward_step
    data = train_data if split == 'train' else val_data
    if batch_mode == 'random':
        ix = torch.randint(len(data) - block_size, (batch_size,))
    elif batch_mode == 'linear':
        ix = [forward_step + i * block_size for i in range(batch_size)]
        forward_step += batch_size * block_size
        if forward_step > train_data.shape[0]:
            forward_step = 0
...
# training loop
X, Y = get_batch('train', batch_mode=batch_mode) # fetch the very first batch
t0 = time.time()
...
while True:
    ...
    if iter_num % eval_interval == 0 and master_process:
        ...
        if save_model:
            torch.save(raw_model.state_dict(), os.path.join(out_dir, f"model_{iter_num}.pt"))
    ...
    for micro_step in range(gradient_accumulation_steps):
        ...
        X, Y = get_batch('train', batch_mode=batch_mode)
